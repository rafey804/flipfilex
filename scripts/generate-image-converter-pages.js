#!/usr/bin/env node

const fs = require('fs');
const path = require('path');

// All image format conversions
const imageFormats = ['jpg', 'png', 'webp', 'avif', 'gif', 'bmp', 'tiff', 'ico', 'svg', 'heic', 'psd'];

// Generate all combinations
const conversions = [];

imageFormats.forEach(from => {
  imageFormats.forEach(to => {
    if (from !== to) {
      conversions.push({ from, to });
    }
  });
});

// Add camera RAW formats
const rawFormats = ['raw', 'cr2', 'nef', 'dng'];
const rawTargets = ['jpg', 'png', 'webp', 'tiff'];
rawFormats.forEach(from => {
  rawTargets.forEach(to => {
    conversions.push({ from, to });
  });
});

console.log(`Total conversions to create: ${conversions.length}`);

// Template for page.tsx
const getPageTemplate = (from, to) => {
  const fromUpper = from.toUpperCase();
  const toUpper = to.toUpperCase();
  const slug = `${from}-to-${to}`;

  // Function name formatting
  const fromCamel = from.charAt(0).toUpperCase() + from.slice(1).toLowerCase();
  const toCamel = to.charAt(0).toUpperCase() + to.slice(1).toLowerCase();
  const functionName = `${fromCamel}To${toCamel}Page`;

  const title = `${fromUpper} to ${toUpper} Converter`;

  return `import { Metadata } from 'next';
import ConverterPage from '@/components/ConverterPageTemplate';

export const metadata: Metadata = {
  title: '${title} - Free Online Tool | FlipFileX',
  description: 'Convert ${fromUpper} to ${toUpper} online for free. Fast, secure, and easy-to-use ${fromUpper} to ${toUpper} converter with no file size limits.',
  keywords: ['${from} to ${to}', '${from} converter', '${to} converter', 'convert ${from}', 'online ${from} converter'],
  openGraph: {
    title: '${title} - FlipFileX',
    description: 'Convert ${fromUpper} to ${toUpper} online for free',
    type: 'website',
    url: 'https://flipfilex.com/${slug}',
  },
  twitter: {
    card: 'summary_large_image',
    title: '${title}',
    description: 'Convert ${fromUpper} to ${toUpper} online for free',
  },
  alternates: {
    canonical: 'https://flipfilex.com/${slug}',
  },
};

export default function ${functionName}() {
  return (
    <ConverterPage
      slug="${slug}"
      sourceFormat="${from}"
      targetFormat="${to}"
      category="image"
      title="${title}"
    />
  );
}
`;
};

// Create directories and files
let created = 0;
let skipped = 0;

conversions.forEach(({ from, to }) => {
  const slug = `${from}-to-${to}`;
  const dirPath = path.join(__dirname, '..', 'file', 'src', 'app', slug);
  const filePath = path.join(dirPath, 'page.tsx');

  // Check if already exists
  if (fs.existsSync(filePath)) {
    skipped++;
    return;
  }

  // Create directory
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
  }

  // Write file
  fs.writeFileSync(filePath, getPageTemplate(from, to));
  created++;

  if (created % 10 === 0) {
    console.log(`Created ${created} pages...`);
  }
});

console.log(`\n✅ Done!`);
console.log(`   Created: ${created} new pages`);
console.log(`   Skipped: ${skipped} existing pages`);
console.log(`   Total: ${conversions.length} converter pages\n`);
